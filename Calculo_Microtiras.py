...
def w_microstrip(e_r, H, t, Z, f):
    A = (Z / 60) * np.sqrt((e_r + 1) / 2) + ((e_r - 1) / (e_r + 1)) * (0.23 + (0.11 / e_r))
    B = (377 * np.pi) / (2 * Z * np.sqrt(e_r))

    W_H = (8 * np.exp(A)) / (np.exp(2 * A) - 2)

    if W_H > 2:
        W_H = (2 / np.pi) * (B - 1 - np.log(2 * B - 1)) + ((e_r - 1) / (2 * e_r)) * (
            np.log(B - 1) + 0.39 - (0.61 / e_r)
        )
    ...
    if W_H >= 1:
        ...
        Zo = ((120 * np.pi) / np.sqrt(e_rp)) / (W_H + 1.393 + 0.667 * np.log(W_H + 1.444))
    else:
...
# Cargar archivo S2P
file = './BFP640/' + f'BFP640_w_noise_VCE_{VCE:.1f}V_IC_{IC:.0f}mA.s2p'
